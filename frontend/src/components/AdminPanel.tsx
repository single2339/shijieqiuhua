...
import { X, Users, Ticket, ChartBar, CaretLeft, Trash, CheckCircle, XCircle, Plus, Key } from '@phosphor-icons/react'
  ...
  if (error) {
    return (
      <div style={{ padding: 40, textAlign: 'center' }}>
        ...
        <button onClick={load} style={{ background: 'rgba(217,107,98,0.10)', border: '1px solid rgba(217,107,98,0.25)', color: 'var(--danger)', padding: '4px 12px', borderRadius: 'var(--radius-sm)', cursor: 'pointer', fontSize: 11, fontFamily: 'var(--font-mono)' }}>重试</button>
      </div>
    )
  }
            ...
            <tr key={u.id}>
              ...
              <td style={tdStyle}>
                <span style={{
                  ...
                  background: u.role === 'admin' ? 'var(--accent-dim)' : 'rgba(236,230,218,0.04)',
                ...
                <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                  <motion.button
                    ...
                    style={{
                      ...
                      background: u.is_active ? 'rgba(217,107,98,0.08)' : 'rgba(79,188,141,0.08)',
                      border: `1px solid ${u.is_active ? 'rgba(217,107,98,0.18)' : 'rgba(79,188,141,0.18)'}`,
      ...
      <div style={{
        ...
        <motion.button
          ...
          style={{
            ...
            color: generating ? 'var(--text-tertiary)' : 'var(--bg-deep)',
...
function StatsTab() {
  ...
  const statCards = [
    ...
    { label: '今日登录', value: stats.today_logins ?? 0 },
    { label: '今日操作', value: stats.today_actions ?? 0 },
  ]
