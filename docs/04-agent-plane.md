...
- 将 Silver/Gold 数据转化为**可审计**的 `Claim` 与分析产物；**禁止**无来源的「裸结论」。
...
| 能力 | 说明 |
|------|------|
| 任务分解 | 将用户或系统触发拆为：抽取 Claim、验真、翻译、经济专题分析 |
...
| Agent | 输入 | 输出 |
|-------|------|------|
| **FactExtract** | `silver_document` | 候选 `Claim` + `evidence_span` |
| **Provenance** | `Claim` | 库内多文档检索，候选 `supporting_silver_document_ids` |
| **Contradiction** | `Claim` + 支持文档集 | `verification_status` 建议、`dispute_notes` |

验真结果**必须**写回 `gold_claim`，不得仅存于对话。

## 4. 经济垂直子智能体

| Agent ID | 职责 | 主要 Gold/Silver 输入 |
|----------|------|------------------------|
