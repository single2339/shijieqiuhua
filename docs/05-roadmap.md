...
| 工作项 | 交付 |
|--------|------|
| Silver | 实体表、去重指纹、基础 NER/链接（规则或模型） |
| Gold | `silver_event`、仪表盘查询；OpenSearch 投影 |
