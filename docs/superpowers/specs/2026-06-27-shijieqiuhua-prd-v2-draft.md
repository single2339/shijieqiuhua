...
| Q-v2-3 | 因子命中/偏差的判定逻辑是否足够准确？ | ✅ **已决定：规则版**。因子 direction 与 actual_outcome 一致 → hit，否则 → miss；不引入 LLM，v2.1 再评估 |
| Q-v2-4 | `info_insufficient` 的比赛在历史列表怎么展示？ | ✅ **已决定：展示，标注"证据不足，未作方向判断"，不计入命中率** |
| Q-v2-5 | bronze_storage 里 job 的 factors 可能因缓存清理丢失，降级策略？ | ✅ **已决定：降级展示"因子数据已过期"，命中标记（lean_correct / scoreline_hit）仍正常展示** |
